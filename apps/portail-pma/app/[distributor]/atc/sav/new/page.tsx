...
import NewSAVEntry from "@/components/atc/sav/NewSAVEntry";

export default async function AtcNewSAVPage({ params }: { params: Promise<{ distributor: string }> }) {
  const { distributor } = await params;
  const user = await requireAccess({ allowedRoles: ["atc"], distributorSlug: distributor });

  const [clients, stores, activeTools] = await Promise.all([
  ...
  const allStores   = stores.map((s) => ({ id: s.id, name: s.name, code: s.code }));
