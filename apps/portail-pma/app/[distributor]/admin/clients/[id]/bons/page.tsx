...
export default async function AdminClientBonsPage({
  params,
  ...
  const [client, bons] = await Promise.all([
    ...
    prisma.bons.findMany({ where: { client_id: id, distributor_id: currentUser.distributorId }, orderBy: { created_at: "desc" }, take: 100 }),
  ]);
  if (!client) notFound();
  ...
  return (
    <AdminModulePage
      badge="Client · Bons"
      title={`Bons · ${client.name}`}
      description={`${bons.length} bon${bons.length > 1 ? "s" : ""} rattaché${bons.length > 1 ? "s" : ""} au client.`}
      ...
      <section style={{ borderRadius: "1.5rem", background: "rgba(255,255,255,0.75)", border: "1px solid rgba(217,227,240,0.9)", overflow: "hidden", boxShadow: "0 8px 24px rgba(15,23,42,0.05)" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: "640px" }}>
            <thead><tr><th style={th}>Numéro</th><th style={th}>Type</th><th style={th}>Statut</th><th style={th}>Priorité</th><th style={th}>Créé le</th><th style={{ ...th, textAlign: "right" }}>Fiche</th></tr></thead>
            <tbody>
              {bons.map((b) => {
                ...
                return (
                  <tr key={b.id}>
                    ...
                    <td style={tdM}>{b.priority || "—"}</td>
                    <td style={tdM}>{formatDate(b.created_at)}</td>
                    <td style={{ ...td, textAlign: "right" }}><Link href={`${adminBase}/bons/${b.id}`} style={btnLink}>Voir</Link></td>
                  </tr>
                );
              })}
              {!bons.length && <tr><td colSpan={6} style={{ ...td, textAlign: "center", color: "#94a3b8", padding: "2.5rem" }}>Aucun bon pour ce client.</td></tr>}
            </tbody>
          </table>
        </div>
      </section>
