...
export default async function AdminCdvDetailPage({
  params,
}: {
  ...
  const cdv = await prisma.users.findFirst({
    where: {
      id,
  ...
  return (
    <AdminModulePage
      badge="CDV · Détail"
      title={`${cdv.first_name} ${cdv.last_name}`.trim()}
      description="Fiche détaillée du CDV, de son portefeuille et de son activité."
    ...
    >
      <section className="card-lysma" style={{ padding: "2rem" }}>
        <div style={{ display: "grid", gap: ".5rem" }}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: "1rem" }}>
            <div>
              <h2 className="section-title" style={{ fontSize: "1.25rem" }}>
                {`${cdv.first_name} ${cdv.last_name}`.trim()}
              </h2>
            ...
            <UserRoleBadge
              roleCode={cdv.roles?.code}
              roleLabel={cdv.roles?.label}
            />
          </div>

          <div className="section-copy">Téléphone : {cdv.phone || "—"}</div>
          <div className="section-copy">Code : {cdv.code || "—"}</div>
          <div className="section-copy">
            Statut :{" "}
            <span className={cdv.is_active ? "status-success" : "status-danger"}>
              {cdv.is_active ? "Actif" : "Inactif"}
            </span>
          </div>
          <div className="section-copy">Clients : {cdv._count.clients}</div>
          <div className="section-copy">Bons : {cdv._count.bons}</div>
          <div className="section-copy">
            Magasins liés :{" "}
          ...
          <div style={{ display: "flex", gap: ".75rem", marginTop: "1rem", flexWrap: "wrap" }}>
            <Link
              href={`/${currentUser.distributorSlug}/admin/users/${cdv.id}`}
              className="btn-secondary"
            >
              Voir le compte utilisateur
            </Link>
            <Link
              href={`/${currentUser.distributorSlug}/admin/users/${cdv.id}/edit`}
              className="btn-secondary"
            >
              Modifier le compte
            </Link>
      ...
      <section className="card-lysma" style={{ padding: "2rem" }}>
        <h2 className="section-title">Clients visibles</h2>
